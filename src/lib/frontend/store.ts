/**
 * Global Zustand store — single source of truth for client-side app state.
 *
 * Structured into sections with the same ordering top-to-bottom:
 *
 *   1. Type definition     (the `AppState` interface)
 *   2. Defaults & persistence (localStorage helpers for `uiPrefs`)
 *   3. The store itself    (`useStore = create(...)`)
 *
 * Each state slice follows the same pattern:
 *
 *     // Domain name
 *     fooList: [],
 *     setFooList: (f) => set({ fooList: f }),
 *
 * Slices currently present:
 *   agents, conversations, messages, UI state (sidebar/panels/streaming),
 *   thinking panel, subagent tree, tabs, tags, templates, workflows,
 *   whiteboard, analytics, checkpoints, webhooks, arena, shared memory,
 *   scheduled tasks, notifications, behaviour mode, folders, guardrails,
